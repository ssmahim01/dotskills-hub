...
export default function Subscriptions() {
  ...
  const handleApproveSubmit = async () => {
    ...
    if (!approvalForm.storeName.trim()) newErrors.storeName = 'Store name is required';
    if (!approvalForm.subdomain.trim()) newErrors.subdomain = 'Subdomain is required';
    ...
    try {
      if (selectedSub) {
        const storeResult = storeService.create({
          ...
          phone: '+880000000000',
          package: approvalForm.package as 'starter' | 'business' | 'enterprise',
        });

        if (storeResult.success && storeResult.store) {
          const approved = subscriptionService.approve(selectedSub.id, 'admin');
          if (approved) {
            storeService.update(storeResult.store.id, {
              subscriptionId: approved?.id ?? "",
            });

            setSubscriptions(subscriptionService.getAll());
            setApprovalDialogOpen(false);
            toast.success(`Subscription approved for ${approvalForm.storeName}`);
          }
        }
      }
    } catch (error) {
  ...
  const columns: Column<Subscription>[] = [
    ...
    },
    {
      key: 'email',
      label: 'Email',
      render: (sub) => <span className="text-muted-foreground">{sub.email}</span>,
    },
    {
      key: 'plan',
      label: 'Plan',
    ...
    },
    {
      key: 'amount',
      label: 'Amount',
      render: (sub) => <span className="font-semibold">{formatCurrency(sub.amount)}</span>,
    },
    {
      key: 'status',
      label: 'Status',
      render: (sub) => (
        <Badge
          variant={sub.status === 'pending' ? 'outline' : sub.status === 'approved' ? 'default' : 'destructive'}
    ...
    },
    {
      key: 'createdAt',
      label: 'Date',
      render: (sub) => <span className="text-sm text-muted-foreground">{formatDate(sub.createdAt)}</span>,
    },
    {
      key: 'id',
      label: 'Actions',
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Manage and approve customer subscriptions</p>
        </div>
        <Button>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            {filterStatus === 'pending' && 'Pending subscriptions awaiting approval'}
      ...
      <Dialog open={approvalDialogOpen} onOpenChange={setApprovalDialogOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Create a store for {selectedSub?.name}. Fill in the store details below.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              {approvalErrors.storeName && (
                <p className="text-xs text-destructive mt-1">{approvalErrors.storeName}</p>
              )}
            </div>
            ...
            <div>
              ...
              <div className="flex gap-2">
                <Input
                  ...
                  onChange={(e) => setApprovalForm({ ...approvalForm, subdomain: e.target.value })}
                  className={approvalErrors.subdomain ? 'border-destructive' : ''}
              ...
              {approvalErrors.subdomain && (
                <p className="text-xs text-destructive mt-1">{approvalErrors.subdomain}</p>
              )}
            </div>
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => setApprovalForm({ ...approvalForm, package: e.target.value })}
