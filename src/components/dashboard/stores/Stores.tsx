...
export default function Stores() {
  ...
  const handleEditSubmit = async () => {
    ...
    if (!editForm.storeName.trim()) newErrors.storeName = 'Store name is required';
    ...
    try {
      if (selectedStore) {
        const updated = storeService.update(selectedStore.id, editForm);
        if (updated) {
          setStores(storeService.getAll());
  ...
  const handleToggleStatus = (store: Store) => {
    ...
    storeService.update(store.id, { status: newStatus });
    setStores(storeService.getAll());
  ...
  const columns: Column<Store>[] = [
    {
      key: 'storeName',
      label: 'Store Name',
      render: (store) => <span className="font-semibold">{store.storeName}</span>,
    },
    {
      key: 'subdomain',
      label: 'Domain',
    ...
    },
    {
      key: 'ownerName',
      label: 'Owner',
      render: (store) => <span className="text-muted-foreground">{store.ownerName}</span>,
    },
    {
      key: 'package',
      label: 'Package',
    ...
    },
    {
      key: 'createdAt',
      label: 'Created',
      render: (store) => <span className="text-sm text-muted-foreground">{formatDate(store.createdAt)}</span>,
    },
    {
      key: 'id',
      label: 'Actions',
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Manage all your e-commerce stores</p>
        </div>
        ...
        <Card>
          <CardContent className="pt-6">
            <div className="space-y-2">
              ...
              <p className="text-3xl font-bold">{stores.filter((s) => s.status === 'active').length}</p>
            </div>
          </CardContent>
        </Card>
        <Card>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Manage your e-commerce stores and their settings</CardDescription>
        </CardHeader>
        <CardContent>
          {isLoading ? (
            <div className="space-y-2">
      ...
      <Dialog open={editDialogOpen} onOpenChange={setEditDialogOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>Update store details and settings.</DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              {editErrors.storeName && (
                <p className="text-xs text-destructive mt-1">{editErrors.storeName}</p>
              )}
            </div>
